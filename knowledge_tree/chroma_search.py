...
def get_embedding_function():
    ...
    if _embedding_function is None:
        ...
        # 可选：强制使用离线 hash embedding（避免任何外网依赖）
        mode = str(os.getenv("KNOWLEDGE_TREE_EMBEDDING_MODE", "")).strip().lower()

        class _HashEmbeddingFunction:
            """本地离线 embedding：基于 token 哈希的稀疏计数向量。

            说明：
            - 优点：完全离线、无外部依赖、稳定可复现。
            - 缺点：语义效果不如 bge 等模型，但足够支撑链路跑通与基本召回。
            """
            ...
            def _normalize_vectors(self, vecs: Any) -> List[List[float]]:
            ...
            def embed_query(self, input: Any, **kwargs) -> List[float]:
                if isinstance(input, list):
                    ...
                    text = input
                vecs = self._normalize_vectors(self([text]))
                return vecs[0] if vecs else []

            def embed_documents(self, input: List[str], **kwargs) -> List[List[float]]:
                return self._normalize_vectors(self(list(input)))

            def __call__(self, input: List[str]) -> List[List[float]]:  # chromadb expects __call__
                ...
                for text in input:
                    ...
                    s = s.strip()
                    # 兼容中英文：中文单字 + 英文/数字连续串
                    tokens = re.findall(r"[\u4e00-\u9fff]|[a-zA-Z0-9_]+", s)
                    for tok in tokens:
        ...
        if mode == "hash":
            _embedding_function = _HashEmbeddingFunction(dim=int(os.getenv("KNOWLEDGE_TREE_HASH_EMBED_DIM", "384") or "384"))
            return _embedding_function

        # 默认优先 SentenceTransformer（可能在首次 __call__ 时触发 HuggingFace 下载）。
        # 为避免“初始化成功但调用失败”的情况，这里实现 call-time fallback。

        hash_fallback = _HashEmbeddingFunction(dim=int(os.getenv("KNOWLEDGE_TREE_HASH_EMBED_DIM", "384") or "384"))

        class _LazyFallbackEmbeddingFunction:
            ...
            def name(self) -> str:
                # chromadb 可能会在日志/telemetry 中调用 embedding_function.name()
                if self._using_fallback:
                    try:
                        n = getattr(self._fallback, "name", None)
                        return n() if callable(n) else (str(n) if n is not None else "hash-embedding")
                    except Exception:
                        return "hash-embedding"

                try:
                    n = getattr(self._primary, "name", None)
                    return n() if callable(n) else (str(n) if n is not None else "sentence-transformer")
                except Exception:
            ...
            def embed_query(self, input: Any, **kwargs) -> List[float]:
                # 统一走 __call__，保证触发 call-time fallback
                if isinstance(input, list):
                    text = input[0] if input else ""
                else:
                    text = input
            ...
            def __call__(self, input: List[str]) -> List[List[float]]:
                ...
                try:
                    ...
                    # 网络/代理/HF 下载失败等，自动切换到离线 hash embedding
                    self._using_fallback = True
                    _embedding_function = self._fallback
                    print(f"⚠️ SentenceTransformer embedding 调用失败，已切换到离线 hash embedding: {e}")
                    return self._fallback(input)
        ...
        try:
            st = embedding_functions.SentenceTransformerEmbeddingFunction(model_name=EMBEDDING_MODEL)
            _embedding_function = _LazyFallbackEmbeddingFunction(st, hash_fallback)
        except Exception as e:
            print(f"⚠️ SentenceTransformerEmbeddingFunction 初始化失败，将直接使用离线 hash embedding: {e}")
            _embedding_function = hash_fallback
    return _embedding_function
...
class ContextualCompressionSearch:
    ...
    def search(
        ...
        def _first_or_empty(v: Any) -> list:
            # chroma 通常返回 List[List[...]]；但在异常/空结果时可能返回 []
            if isinstance(v, list) and v:
                return v[0] if v[0] is not None else []
            return []

        return {
            "query": query,
            "documents": _first_or_empty(results.get("documents")),
            "distances": _first_or_empty(results.get("distances")),
            "metadatas": _first_or_empty(results.get("metadatas")),
            "ids": _first_or_empty(results.get("ids"))
        }
