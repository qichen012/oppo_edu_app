...
from database import (
    ...
    ScholarNote, KnowledgeMap, MapInteractionLog, MapCognitiveSnapshot
)
...
class UserBase(BaseModel):
    email: str = Field(..., max_length=100, description="用户邮箱") 
    name: Optional[str] = Field(None, max_length=45)
    gender: Optional[str] = Field(None, pattern="^(male|female)$")
    age: Optional[int] = Field(None, ge=0, le=120)
    current_subject: Optional[str] = Field(None, max_length=100, description="当前科目/主题")
...
class UserUpdate(BaseModel):
    ...
    gender: Optional[str] = Field(None, pattern="^(male|female)$")
    age: Optional[int] = Field(None, ge=0, le=120)
    current_subject: Optional[str] = Field(None, max_length=100)
...
def register(user_data: UserCreate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return TokenResponse(token=access_token, message="注册成功", code=200, user_id=new_user.id)
...
def login(user_data: UserAuthRequest, db: Session = Depends(get_db)):
    ...
    return TokenResponse(token=access_token, message="登录成功", code=200, user_id=user.id)
...
@api_router.get("/app-usage/stats/{user_id}", response_model=AppUsageStatsResponse)
def get_usage_stats(user_id: int, db: Session = Depends(get_db)):
    today = datetime.now().date()
    
    results = db.query(
        func.hour(AppUsageLog.start_time).label("hour_slot"),
        func.sum(AppUsageLog.duration_seconds).label("total_duration")
    ).filter(
        AppUsageLog.user_id == user_id,
        func.date(AppUsageLog.start_time) == today
    ).group_by(func.hour(AppUsageLog.start_time)).all()
    ...
    for row in results:
        hour = row.hour_slot
        if 0 <= hour < 24:
            minutes = round(float(row.total_duration or 0) / 60.0, 1)
            data_points[hour] = minutes
...
class DailyBriefBase(BaseModel):
    user_id: Optional[int] = None
    posterior_insight: Optional[str] = Field(None, max_length=100)
    key_concepts: Optional[str] = None
    created_at: Optional[datetime] = None
    review_stage: Optional[int] = None
    User_reflect: Optional[str] = None
    source_handouts: Optional[str] = Field(None, max_length=100)
    origin: Optional[str] = Field(None, pattern="^(SC|PDF)$")
...
class EliteIdeaCardBase(BaseModel):
    ...
    origin_concept: Optional[str] = Field(None, max_length=100)
    meta_idea_name: Optional[str] = Field(None, max_length=100)
    meta_explanation: Optional[str] = Field(None, max_length=100)
...
async def upload_pdf(
    ...
    user = None
    if user_email:
        user = db.query(UserInformation).filter(UserInformation.email == user_email).first()
        if not user:
            raise HTTPException(status_code=404, detail="User not found")
    ...
    doc_data = {
        ...
        "processed_status": "Pending"  # 新增字段
    }
    
    db_doc = SourceDocument(**doc_data)
    db.add(db_doc)
    db.commit()
    db.refresh(db_doc)
    ...
    return {
        ...
        "processed_status": db_doc.processed_status
    }
...
def create_daily_brief(brief: DailyBriefCreate, db: Session = Depends(get_db)):
    try:
        ...
        db.rollback()
        detail = str(getattr(e, "orig", "") or e)
        raise HTTPException(status_code=400, detail=f"DB integrity error: {detail}")
...
class EliteIdeaCaseBase(BaseModel):
    ...
    case_title: Optional[str] = Field(None, max_length=100)
    case_content: Optional[str] = Field(None, max_length=100)
    image_path: Optional[str] = Field(None, max_length=100)
...
class ExternalResourceBase(BaseModel):
    ...
    url: Optional[str] = Field(None, max_length=100)
...
class ScholarNoteBase(BaseModel):
    user_id: Optional[int] = None
    daily_brief_id: Optional[int] = None
    note_title: Optional[str] = Field(None, max_length=100)
